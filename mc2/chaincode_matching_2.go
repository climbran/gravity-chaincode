...
//test case
//['mc','{"kkkk":{"PublishTime":"2018-08-27T12:31:47Z","City":"上海","Price":100},"nnnnn":{"PublishTime":"2019-06-27T12:31:47Z","City":"北京","Price":300}}',"北京","50","300"]
...
func (infos Infos) Less(i, j int) bool {
	return infos[i].Price < infos[j].Price
}
...
type D2_MatchingChaincode struct {
}

func (t *D2_MatchingChaincode) Init(stub shim.ChaincodeStubInterface) pb.Response {
	//目前不支持跨链码交易，需手动注册
	//checkResponse := stub.InvokeChaincode("matching", [][]byte{[]byte("signup"), []byte("2"), []byte("matching_2")}, "")

	return shim.Success([]byte("ok"))
}

func (t *D2_MatchingChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
...
func (t *D2_MatchingChaincode) matching(stub shim.ChaincodeStubInterface, infos_str, city string, price_lower, price_upper int) pb.Response {
	var infos Infos
...
func main() {
	err := shim.Start(new(D2_MatchingChaincode))
	if err != nil {
		fmt.Println("Could not start D2_MatchingChaincode")
	} else {
		fmt.Println("D2_MatchingChaincode successfully started")
	}
	// u := &User{"Yan", 18}
	// fmt.Printf("%s\n", u.toString())
}
